...
select sum(p.len*p.width*p.height*product_quantity) as volume from orders.order_header oh
...
select oc.customer_id, concat(customer_fname,'',customer_lname) as Customer_full_name, oh.order_id,
sum(oi.product_quantity) as Total_Order_Qty from online_customer oc
...
select oh.order_id, oc.customer_id, concat(customer_fname,'',customer_lname) as Customer_full_name, 
sum(oi.product_quantity) as product_Qty from online_customer oc
inner join order_header oh on oh.customer_id=oc.customer_id
inner join order_items oi on oi.order_id=oh.order_id
